...
//             if(Math.abs(m_rotate.sliderLdrMtr.getPosition().getValueAsDouble() - angle) <= 1 && Math.abs(m_outtake.wheelSpeedMtr.getVelocity().getValueAsDouble() - ConstantValues.SHOOTER_RPM) <= 30 && Math.abs(drivetrain.getState().Pose.getRotation().getRadians() - yaw) <= 0.5){
